...
//   courseTitle: string;
//   syllabusUrl?: string;
//   syllabusFile?: File; // Added to store the actual File object
//   l: string;
//   t: string;
//   p: string;
//   c: string;
...
//   verticalName: string;
//   cells: { courseCode: string; courseTitle: string }[];
// }
...
//   isSemester: boolean;
//   onChange: (index: number, field: string, value: string | File) => void;
//   onRemove: (index: number) => void;
//   canRemove: boolean;
//   allSubjects: { _id: string; title: string; code: string; syllabusUrl?: string }[];
// }> = ({ allSubjects, category, index, course, isSemester, onChange, onRemove, canRemove }) => {
...
//       try {
//         // Construct URL for /api/auth/file/:id
//         const baseUrl = process.env.REACT_APP_API_BASE_URL || 'http://localhost:5000';
//         const syllabusUrl = `${baseUrl}/api/auth/file/${selectedSubject.syllabusUrl}`;
//         console.log('Fetching syllabus from:', syllabusUrl); // Debug
...
//   return (
//     <div className="flex items-center space-x-2 py-2 bg-purple-50 rounded-lg p-3">
//       <input
//         type="text"
//         value={course.sno}
...
//       />
//       <select
//         value={allSubjects.find((subj) => subj.code === (course as SemesterCourse).courseCode)?._id || ''}
...
//   const [fetchedSyllabusFiles, setFetchedSyllabusFiles] = useState<{ [key: string]: Blob }>({});
//   const [selectedSyllabusFiles, setSelectedSyllabusFiles] = useState<File[]>([]);
...
//     semester1Courses: [{ sno: '1', type: '', courseCode: '', courseTitle: '', syllabusUrl: '', l: '', t: '', p: '', c: '' }],
//     semester2Courses: [{ sno: '1', type: '', courseCode: '', courseTitle: '', syllabusUrl: '', l: '', t: '', p: '', c: '' }],
//     semester3Courses: [{ sno: '1', type: '', courseCode: '', courseTitle: '', syllabusUrl: '', l: '', t: '', p: '', c: '' }],
//     semester4Courses: [{ sno: '1', type: '', courseCode: '', courseTitle: '', syllabusUrl: '', l: '', t: '', p: '', c: '' }],
//     semester5Courses: [{ sno: '1', type: '', courseCode: '', courseTitle: '', syllabusUrl: '', l: '', t: '', p: '', c: '' }],
//     semester6Courses: [{ sno: '1', type: '', courseCode: '', courseTitle: '', syllabusUrl: '', l: '', t: '', p: '', c: '' }],
//     semester7Courses: [{ sno: '1', type: '', courseCode: '', courseTitle: '', syllabusUrl: '', l: '', t: '', p: '', c: '' }],
//     semester8Courses: [{ sno: '1', type: '', courseCode: '', courseTitle: '', syllabusUrl: '', l: '', t: '', p: '', c: '' }],
//     professionalElectives: [
//       {
//         verticalNumber: 'Vertical I',
//         verticalName: '',
//         cells: [{ courseCode: '', courseTitle: '' }],
//       },
//     ],
//   });
...
//   // Collect all syllabus files from semester courses
//   useEffect(() => {
...
//     const files = allSemesterCourses
//       .filter((course) => course.syllabusFile)
//       .map((course) => course.syllabusFile!);

//     // Remove duplicates based on file name
//     const uniqueFiles = Array.from(new Map(files.map((file) => [file.name, file])).values());
//     setSelectedSyllabusFiles(uniqueFiles);
//   }, [
//     formFields.semester1Courses,
//     formFields.semester2Courses,
//     formFields.semester3Courses,
...
//           ? { sno: String(i + 1), type: '', courseCode: '', courseTitle: '', syllabusUrl: '', l: '', t: '', p: '', c: '' }
...
//     columnIndex: number,
//     rowIndex: number,
//     field: 'courseCode' | 'courseTitle',
//     value: string
//   ) => {
//     setFormFields((prev) => {
//       const updated = [...prev.professionalElectives];
//       updated[columnIndex].cells[rowIndex][field] = value;
//       return { ...prev, professionalElectives: updated };
//     });
//   };
...
//           verticalName: '',
//           cells: Array(formFields.professionalElectives[0].cells.length).fill({ courseCode: '', courseTitle: '' }),
//         },
//       ],
//     }));
//   };
...
//         ...col,
//         cells: [...col.cells, { courseCode: '', courseTitle: '' }],
//       }));
//       return { ...prev, professionalElectives: updated };
//     });
//   };
...
//   const generateElectivesTable = (electives: ElectiveColumn[]): Table => {
//     const rows: TableRow[] = [];
//     const columnCount = electives.length;
...
//                 children: [
//                   new TextRun({ text: sanitize(col.verticalNumber), bold: true }),
//                 ],
//               }),
//               new Paragraph({
//                 alignment: AlignmentType.CENTER,
//                 children: [
//                   new TextRun({ text: sanitize(col.verticalName || ''), bold: true }),
//                 ],
//               }),
//             ],
//           })
...
//       const rowCells = electives.map((col) => {
//         const cell = col.cells[i] || { courseCode: '', courseTitle: '' };
...
//               children: [
//                 new TextRun({
//                   text: sanitize(cell.courseTitle || ''),
//                 }),
//               ],
//             }),
//           ],
...
//     try {
//       // 1. Fill curriculum template
//       const data = transformFormFields(formFields);
//       const docxBlob = await generateDocxFromTemplate(
//         '/templates/Curriculum-Template.docx',
//         data
//       );
//       const buffer1 = await docxBlob.arrayBuffer();

//       // 2. Create electives section
...
//       // 3. Merge using correct API
//       const merger = new DocxMerger();
//       await merger.merge([buffer1, buffer2]);
//       const mergedArrayBuffer = await merger.save();

//       // 4. Save final .docx
//       const finalBlob = new Blob([mergedArrayBuffer], {
//         type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
//       });
//       saveAs(finalBlob, 'Curriculum.docx');

//       toast.success('✔️ Curriculum downloaded successfully');
//     } catch (error) {
...
//   const handleGenerateFullCurriculum = async () => {
//     setIsGenerating(true);

//     try {
//       // 1. Generate curriculum base document
//       const data = transformFormFields(formFields);
//       const curriculumBlob = await generateDocxFromTemplate(
//         '/templates/Curriculum-Template.docx',
//         data
//       );
//       const curriculumBuffer = await curriculumBlob.arrayBuffer();

//       // 2. Generate electives table
//       const electives = formFields.professionalElectives;
//       const columnWidth = 2460;
//       const maxRows = Math.max(...electives.map(col => col.cells.length));
...
//               new Paragraph({
//                 alignment: AlignmentType.CENTER,
//                 children: [new TextRun({ text: col.verticalNumber, bold: true, font: 'Cambria', size: 22 })],
//               }),
//               new Paragraph({
//                 alignment: AlignmentType.CENTER,
//                 children: [new TextRun({ text: col.verticalName, bold: true, font: 'Cambria', size: 22 })],
//               }),
//             ],
//           })
//         ),
...
//                 new Paragraph({
//                   alignment: AlignmentType.CENTER,
//                   children: [new TextRun({ text: c.courseCode, bold: true, font: 'Cambria', size: 22 })],
//                 }),
//                 new Paragraph({
//                   alignment: AlignmentType.CENTER,
...
//         children: [
//           new TextRun({
//             text: '*Students are permitted to choose all the Professional Electives from a particular vertical or from different verticals. However, Students are restricted to select from not more than 2 verticals.',
//             font: 'Cambria',
//             size: 22,
//           }),
//         ],
...
//       const electivesDoc = new Document({ sections: [{ children: [heading, electivesTable, note] }] });
//       const electivesBlob = await Packer.toBlob(electivesDoc);
//       const electivesBuffer = await electivesBlob.arrayBuffer();

//       // 3. Use selected syllabus files from state
//       const syllabusBuffers: ArrayBuffer[] = await Promise.all(
//         selectedSyllabusFiles.map(file => file.arrayBuffer())
//       );

//       // 4. Merge everything
//       const merger = new DocxMerger();
//       await merger.merge([curriculumBuffer, electivesBuffer, ...syllabusBuffers]);
...
//       formData.append("file", finalBlob, "Curriculum.docx");
...
//       const regulationCode = formFields.regulation;
//       const department = formFields.branchName;

//       console.log(regulationCode);
//       console.log(department);

//       if (!fileId) throw new Error("File upload failed - no fileId returned");

//       // Link file to subject and update status
//       const res = await fetch("http://localhost:5000/api/auth/upload-curriculum", {
//         method: "PUT",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ regulationCode, department, fileId }),
//       });
...
//   const renderFullCurriculumSection = () => (
//     <div className="space-y-6">
//       <div className="rounded-md border dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
//         <div className="bg-gradient-to-r from-purple-500 to-purple-600 text-white p-4 rounded-t-md">
//           <h3 className="text-lg font-semibold">Full Curriculum Merge</h3>
//         </div>
//         <div className="p-6 space-y-4">
//           {selectedSyllabusFiles.length > 0 ? (
//             <div className="mt-2 space-y-1 text-sm text-gray-800 dark:text-gray-300">
//               <p className="font-semibold">Selected Syllabus Files:</p>
//               <ul className="list-disc pl-5">
//                 {selectedSyllabusFiles.map((file, i) => (
...
//               No syllabus files selected. Please select subjects with syllabus files in the Semester Courses section.
//             </p>
//           )}
//           <div className="text-center">
//             <Button
//               onClick={handleGenerateFullCurriculum}
//               className="bg-purple-600 hover:bg-purple-700 text-white"
//               disabled={isGenerating || selectedSyllabusFiles.length === 0}
//             >
//               {isGenerating ? 'Generating...' : 'Generate Full Curriculum DOCX'}
//             </Button>
//           </div>
//         </div>
//       </div>
...
//             >
//               {isGenerating ? 'Generating...' : 'Generate Professional Electives DOCX'}
//             </Button>
//           </div>
//         </div>
//       </div>
...
//             { key: 'fullcurriculum', label: 'Full Curriculum' },
//           ].map(({ key, label }) => (
//             <button
//               key={key}
...
//             {activeSection === 'fullcurriculum' && renderFullCurriculumSection()}
//           </div>
//         </div>
//       </div>
//     </div>
...
const CreateCurriculum: React.FC = () => {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      // 1. Generate base curriculum from template
      const data = transformFormFields(formFields);
      ...
      // 3. Collect syllabus files from semester courses and professional electives
      const allSemesterCourses = [
        ...formFields.semester1Courses,
        ...formFields.semester2Courses,
        ...formFields.semester3Courses,
      ...
      // 5. Merge curriculum, electives, and syllabus files
      const merger = new DocxMerger();
            ...
            <div className="space-y-2">
              ...
              {courses.map((course, index) => (
                <CourseInputRow
                  ...
                  onRemove={() => handleRemoveCourse(category, index)}
                  canRemove={courses.length > 1}
                  allSubjects={[]} // No subject selection for Category Courses
                />
              ))}
            </div>
            <div className="space-y-2">
